...
if (localStorage.key(0)) {
  for (let index = 0; index <= localStorage.key(0); index++) {
    console.log(localStorage.getItem(localStorage.key(index)));
    cont = parseInt(localStorage.key(0)) + 1;
...
class user {
  ...
  static total = cont;
  id = 0;
  constructor(Name, Email, Phone, User_password) {
...
btn.addEventListener("click", function (e) {
  ...
  if (((ValidateName(FullName))) && ((ValidateNumber(phoneNumber))) && ((ValidateEmail(Email))) && ((ValidatePasswords(password1, password2)))) {
    console.log("se ha creado una clasecon nombre : " + FullName);
    localStorage.setItem(String(cont), JSON.stringify(FullName = new user(FullName, phoneNumber, Email, password1)));
    cont++;
    Swal.fire({
      position: 'center',
      icon: 'success',
  ...
  console.log(FullName.id, FullName.Name, FullName.Phone, FullName.Email, FullName.User_password);
